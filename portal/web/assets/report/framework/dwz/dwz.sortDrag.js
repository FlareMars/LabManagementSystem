...
(function($){
	var _op = {
		...
		zIndex: 1000,
		stopCB:function($currentItem){}  //拖动结束后的回调函数(custom)
	};
	var sortDrag = {
	...
	$.fn.sortDrag = function(options){
		var $containers=this.each(function(){
			 _op = $.extend({}, _op, options);//custom
			var $sortBox = $(this);
			
			if ($sortBox.attr('selector')) _op.selector = $sortBox.attr('selector');
			$sortBox.find(_op.items).each(function(i){
				var $item = $(this), $selector = $item;
				if (_op.selector) {
					$selector = $item.find(_op.selector).css({cursor:_op.cursor});
				}

				$selector.mousedown(function(event){
					sortDrag.start($sortBox, $item, event, _op);
		...
		return {
			...
			sortDrag:sortDrag,
			//为指定的对象去除拖拽功能
			disableDragFeature:function($eleList){
				if(!$eleList || $eleList.size()===0){
					return;
				}
				$eleList.off();
			},
			...
			//为指定的对象激活拖拽功能
			enableDragFeature:function(opt,$container,$eleList){
				//注意：不要采用代理模式来优化，会出异常
				$eleList.on("mousedown",function(e){
					sortDrag.start($container, $(this), e, opt);
				});
			}
		};
	}
