...
+function ($) {
  ...
  Validator.prototype.showErrors = function ($el) {
    function callback() {
      ...
      var $errorStructure=$block.find(".list-unstyled");
      var errors = $el.data('bs.errors')
      var errorTemplate = '<ul class="list-unstyled"><li>{error-infor}</li></ul>';
...
/*
      ...
      /*$block.empty().append(errors);*/
         ...
         if($errorStructure.length===0){
                errors=errorTemplate.replace("{error-infor}",errors);
                $block.html(errors);
         }else{
             $errorStructure.find("li:first-child").text(errors);
         }
  ...
  Validator.prototype.clearErrors = function ($el) {
    ...
    /*$block.html($block.data('bs.originalContent'))*/
     //Extend Begin
    var originalContent = $block.data('bs.originalContent');
    if(originalContent ==undefined ||  originalContent.replace(" ","").length===0 ){
        $block.hide();
    }else{
        $block.html(originalContent);
    }
    //Extend End
